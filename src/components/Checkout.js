  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e)=>handleNewAddress((prevState)=>({...prevState,value:e.target.value}))}
      />
...
let OrderDetailsComponent=({items,getTotalCartValue})=>{

  return (
    <Box className="order-details" padding="1rem">
      <Box><h3>Order Details</h3></Box>
      <Box display="flex" justifyContent="space-between"><p className="order-attribute">Products</p><p className="order-attribute">{getTotalItems(items)}</p></Box>
      <Box display="flex" justifyContent="space-between"><p className="order-attribute">Subtotal</p><p className="order-attribute">${getTotalCartValue(items)}</p></Box>
      <Box display="flex" justifyContent="space-between"><p className="order-attribute">Shipping Charges</p><p className="order-attribute">$0</p></Box>
      <Box display="flex" justifyContent="space-between"><h4 className="total">Total</h4><h4 className="total">${getTotalCartValue(items)}</h4></Box>
    </Box>
  )
}
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setAddresses((prevState)=>({...prevState, all:res.data}));
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      let res=await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,{headers:{Authorization:`Bearer ${token}`}})
      setAddresses((prevState)=>({...prevState,all:res.data,selected:""}));
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const validateRequest = (items, addresses) => {
    let totalAmount=getTotalCartValue(items);
    if(totalAmount>localStorage.getItem("balance")){
  ...
  const performCheckout = async (token, items, addresses) => {
    if(validateRequest(items,addresses)){
      try{
        let res=await axios.post(`${config.endpoint}/cart/checkout`,{addressId:addresses.selected._id},{headers:{Authorization:`Bearer ${token}`}});
        let balance=localStorage.getItem("balance");
        let totalAmount=getTotalCartValue(items);
        localStorage.setItem("balance",Number(balance)-totalAmount);
        history.push("/thanks");
      }catch(err){
  ...
  return (
    <>
      ...
      <Grid container>
            ...
            <Box>
              ...
              :
               addresses.all.map((item)=>{
                return(
                  <Box key={item._id} className={`address-item ${
                    item._id === addresses.selected._id
                      ? "selected"
                      : "not-selected"
                  }`}
                  onClick={() =>{
                  ...
                  <Button onClick={()=>deleteAddress(token,item._id)}><Delete/><Typography sx={{color:"#00a278"}}>Delete</Typography></Button>
                  </Box>
                )
               })
              }
        ...
        <Grid item xs={12} md={4} lg={3} bgcolor="#E9F5E1">
          <Cart isReadOnly products={products} items={items} />
          <OrderDetailsComponent items={items} getTotalCartValue={getTotalCartValue}/>
        </Grid>
      </Grid>
      <Footer />
    </>
